  ...
  if (reqUrl === "/") {
    ...
    try {
      ...
      const response = [
        ...
        `Number of students: ${(students[0]?.length || 0) +
          (students[1]?.length || 0)}`,
        `Number of students in CS: ${students[0]?.length ||
          0}. List: ${students[0]?.join(", ") || ""}`,
        `Number of students in SWE: ${students[1]?.length ||
          0}. List: ${students[1]?.join(", ") || ""}`
      ].join("\n");
